...
struct dynamic_converter<std::tuple<Args...>, void> {
    ...
    convert(const dynamic_t& from) {
        if (sizeof...(Args) == from.as_array().size()) {
            if (sizeof...(Args) == 0) {
                ...
                return range_applier<sizeof...(Args) - 1>::conv(from);
            }
        } else {
            throw std::bad_cast();
        }
    ...
    bool
    convertible(const dynamic_t& from) {
        if (from.is_array() && sizeof...(Args) == from.size()) {
            if (sizeof...(Args) == 0) {
                return true;
            } else {
                return range_applier<sizeof...(Args) - 1>::is_conv(from);
            }
        } else {
            return false;
        }
...
private:
    ...
    struct range_applier<0, Idxs...> {
        ...
        inline
        result_type
        conv(const dynamic_t& from) {
            const dynamic_t::array_t& arr = from.as_array();
            return std::tuple<Args...>(
                arr[0].to<typename std::tuple_element<0, result_type>::type>(),
                arr[Idxs].to<typename std::tuple_element<Idxs, result_type>::type>()...
            );
        }
